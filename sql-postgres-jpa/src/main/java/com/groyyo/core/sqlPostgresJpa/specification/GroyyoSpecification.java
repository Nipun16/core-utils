package com.groyyo.core.sqlPostgresJpa.specification;

import com.groyyo.core.base.GroyyoConstants;
import com.groyyo.core.sqlPostgresJpa.entity.AbstractJpaEntity;
import com.groyyo.core.sqlPostgresJpa.specification.utils.SearchCriteria;
...
public class GroyyoSpecification<T extends AbstractJpaEntity> implements Specification<T> {

	private static final long serialVersionUID = 1L;

	private SearchCriteria criteria;

	public GroyyoSpecification(SearchCriteria sc) {
		criteria = sc;
	}
		...
		switch (criteria.getOperation()) {
			...
			case DATE_EQ:
				if (criteria.getValue() instanceof LocalDate) {
					return builder.equal(root.get(criteria.getKey()), Date.from(((LocalDate) criteria.getValue()).atStartOfDay(ZoneId.of(GroyyoConstants.IST_TIMEZONE)).toInstant()));
				} else {
			...
			case DATE_LT:
				if (criteria.getValue() instanceof LocalDate) {
					return builder.lessThan(root.get(criteria.getKey()), Date.from(((LocalDate) criteria.getValue()).atStartOfDay(ZoneId.of(GroyyoConstants.IST_TIMEZONE)).toInstant()));
				} else {
					return builder.lessThan(root.get(criteria.getKey()), (Date) criteria.getValue());
				}
			case DATE_GT:
				if (criteria.getValue() instanceof LocalDate) {
					return builder.greaterThan(root.get(criteria.getKey()), Date.from(((LocalDate) criteria.getValue()).atStartOfDay(ZoneId.of(GroyyoConstants.IST_TIMEZONE)).toInstant()));
				} else {
			...
			case DATE_LTE:
				if (criteria.getValue() instanceof LocalDate) {
					return builder.lessThanOrEqualTo(
							root.get(criteria.getKey()), Date.from(((LocalDate) criteria.getValue()).atStartOfDay(ZoneId.of(GroyyoConstants.IST_TIMEZONE)).toInstant()));
				} else {
