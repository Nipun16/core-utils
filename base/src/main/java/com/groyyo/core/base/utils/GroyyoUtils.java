...
public class GroyyoUtils {
	...
	public static String generateUniqueEmailId() {
		return generateUniqueId().replaceAll("-", "") + GroyyoConstants.ORGANIZATION_EMAIL_DOMAIN;
	}
	...
	public static double roundOff(Double price) {
		...
		if (price != null) {
			return roundToPlaces(price, GroyyoConstants.PRICE_ROUND_OFF_DIGITS);
		}
	...
	public static BigDecimal getPercentageValueOf(Number number) {
		return getBigDecimalValueOf(number).divide(getBigDecimalValueOf(100), GroyyoUtils.SCALE,
				GroyyoUtils.ROUNDING_MODE);
	}
	...
	public Float getAvg(Float totalRating, Integer totalRatingCount, int roundToPlaces) {
		...
		if (Objects.nonNull(totalRating) && Objects.nonNull(totalRatingCount) && totalRatingCount > 0) {
			...
			avgRating = GroyyoUtils.roundToPlaces(avgRating, roundToPlaces);
		}
