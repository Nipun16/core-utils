 ...
 * 
 */
package com.groyyo.core.base.http;
...
public class GroyyoRestClient {
	...
	public GroyyoRestClient(String basePath) {
		this.basePath = basePath;
		this.restTemplate = buildRestTemplate();
		objectMapper = BaseMapperConfig.getDefaultMapper();
	}

	public GroyyoRestClient(String basePath, int connectTimeOut, int readTimeOut) {
		this.basePath = basePath;
	...
	public GroyyoRestClient setUserAgent(String userAgent) {
		addDefaultHeader("User-Agent", userAgent);
		return this;
	}

	public GroyyoRestClient addDefaultHeader(String name, String value) {
	...
	private <T> T getResponse(RequestEntity<Object> requestEntity, ParameterizedTypeReference<T> returnType, final UriComponentsBuilder builder) {
		...
		try {
			...
			if (!StringUtils.isEmpty(e.getMessage()) && e.getMessage().contains("401")) {
				...
				throw new GroyyoHttpException(e.getMessage(), e);
			}
		}
		...
		if (responseEntity.getStatusCode() == HttpStatus.NO_CONTENT) {
			...
			throw new GroyyoHttpException("API returned " + statusCode + " and it wasn't handled by the RestTemplate error handler", statusCode.value());
		}
	}
	...
	public <T> T request(
		...
		try {
			...
			throw new GroyyoHttpException("Error while reading response", statusCode.value(), e);
		}
	}
	...
	private <T> T processResponse(TypeReference<T> returnType, UriComponentsBuilder builder, ResponseEntity<String> responseEntity) {
		...
		if (isAccessDenied(statusCode)) {
			throw new GroyyoSecurityException("Access Denied for User", statusCode.value());
		} else {
			// The error handler built into the RestTemplate should handle 400 and 500 series errors.
			throw new GroyyoHttpException("API returned " + statusCode + " and it wasn't handled by the RestTemplate error handler", statusCode.value());
		}
	...
	private void addHeadersToRequest(HttpHeaders headers, BodyBuilder requestBuilder) {
		...
		headers.add(GroyyoConstants.GUID, MDC.get(GroyyoConstants.GUID));
	}
	...
	public GroyyoRestClient addGUIDHeader() {////////////////////////////

		if (defaultHeaders.containsKey(GroyyoConstants.GUID)) {
			defaultHeaders.remove(GroyyoConstants.GUID);
		}

		defaultHeaders.add(GroyyoConstants.GUID, MDC.get(GroyyoConstants.GUID));
